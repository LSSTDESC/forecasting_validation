...
def plot_tomo_peaks_zres_sweep(bin_centers_resolutions,
    ...
    Parameters:
        ...
        extra_info (str): Extra information to add to the figure name (default: "").
    """

    valid_bin_types = ["source", "lens"]
    if bin_type not in valid_bin_types:
        raise ValueError(f"Invalid bin_type '{bin_type}'. Must be one of {valid_bin_types}.")
    ...
    for i, bin_key in enumerate(bin_keys):
        ...
        if precision:
            ...
            axes[i].legend(fontsize=12, loc='upper right', frameon=False)
            ...
            for res, value in zip(resolutions, bin_center_values):
                ...
                if stable_count >= stability_steps:
                    axes[i].axvline(x=res, color='red', linestyle='--', label=f'{res}')
    ...
    fig_name = f"{bin_type}_bin_peaks_zres_sweep_zmax{zmax}_y{forecast_year}{extra}{fig_format}"
...
def plot_tomo_peaks_zres_and_zmax_sweep(bin_centers_by_zmax,
    ...
    Parameters:
        ...
        extra_info (str): Extra information to add to the figure name (default: "").
    """
    # Ensure only valid bin types are used
    valid_bin_types = ["source", "lens"]
    if bin_type not in valid_bin_types:
        raise ValueError(f"Invalid bin_type '{bin_type}'. Must be one of {valid_bin_types}.")
    ...
    # Save the figure
    extra = f"_{extra_info}" if extra_info else ""
    fig_name = f"{bin_type}_stabilization_resolution_heatmap_zmax_sweep_y{forecast_year}{extra}{fig_format}"
    plt.savefig(f"plots_output/{fig_name}")
    plt.show()
...
def plot_kernel_peaks_zres_sweep(peaks_by_resolution,
    ...
    extra = f"_{extra_info}" if extra_info else ""
    fig_name = f"plots_output/{fname_prefix}kernel_peaks_resolution_sweep{extra}{fig_format}"
    plt.savefig(fig_name)
    plt.show()


def plot_kernel_peaks_zres_zmax_sweep(kernel_peaks_by_zmax_and_resolution,
                                                 forecast_year,
                                                 kernel_type,
                                                 precision=0.1,
                                                 stability_steps=10,
...
def plot_gbias_value_zres_zmax_sweep(galaxy_bias_by_zmax_and_resolution,
                                           forecast_year,
                                           precision=0.1,
                                           stability_steps=10,
                                           annotate_max=False,
